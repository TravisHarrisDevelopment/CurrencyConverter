...
def has_error(json):
    ...
    """
    assert type(json)==str
    errStart = introcs.find_str(json, 'error":')
    substring = first_inside_quotes(json[errStart+7:])
    return substring!=""
...
def service_response(src, dst, amt):
    ...
    amount = '&amt=' + str(amt)
    theKey = '&key=' + APIKEY
    query = url + source + destination + amount + theKey
    result = introcs.urlread(query)
    return result
...
def iscurrency(currency):
    ...
    query = url + source + destination + amount + theKey
    result = introcs.urlread(query)
    search = 'The rate for currency ' + currency + ' is not present'
    return not(search in result)
